...
_default_WOFS_size    = [900., 900.]   # x and y dimensions of grid 
...
parser.add_option("-x", "--xsize",   dest="xsize",   type="int",    default=None, help = "West-East length of WOFS domain in km")
parser.add_option("-y", "--ysize",   dest="ysize",   type="int",    default=None, help = "North-South length of WOFS domain in km")
...
parser.add_option("-d", "--hwtdd",  dest="hwtdd",   action="store_true",    default=False,     help="Flag to enable plot of HWT daily domain in background")
...
if options.xsize == None:
    print("\n  No WOFS xgrid width supplied, using the default of %d km \n " % _default_WOFS_size[0])
    WOFS_xsize = 1000. * _default_WOFS_size[0]
else:
    print("\n  WOFS grid width supplied, using the input value of %d km \n " % options.xsize)
    WOFS_xsize = 1000. * options.xsize

if options.ysize == None:
    print("\n  No WOFS xgrid width supplied, using the default of %d km \n " % _default_WOFS_size[1])
    WOFS_ysize = 1000. * _default_WOFS_size[1]
else:
    print("\n  WOFS grid width supplied, using the input value of %d km \n " % options.ysize)
    WOFS_ysize = 1000. * options.ysize
...
glat[0] = lat_c - 0.5*(WOFS_ysize) / __r_earth
glat[1] = lat_c + 0.5*(WOFS_ysize) / __r_earth
glat[2] = lat_c + 0.5*(WOFS_ysize) / __r_earth
glat[3] = lat_c - 0.5*(WOFS_ysize) / __r_earth
glat[4] = lat_c - 0.5*(WOFS_ysize) / __r_earth

glon[0] = lon_c - 0.5*(WOFS_xsize) / (__r_earth * N.cos(glat[0]))
glon[1] = lon_c - 0.5*(WOFS_xsize) / (__r_earth * N.cos(glat[1]))
glon[2] = lon_c + 0.5*(WOFS_xsize) / (__r_earth * N.cos(glat[2]))
glon[3] = lon_c + 0.5*(WOFS_xsize) / (__r_earth * N.cos(glat[3]))
glon[4] = lon_c - 0.5*(WOFS_xsize) / (__r_earth * N.cos(glat[4]))
...
# *** Added by Brett Roberts 2019-04-24 ***
# Attempts to plot red polygon for today's HWT daily domain when -d is used
if options.hwtdd:
    try:
        import datetime
        import json
        ...
        dir_json = '/work/wof/hwt_dd/'
        today_yyyymmdd = datetime.datetime.utcnow().strftime("%Y%m%d")

        path_json = os.path.join(dir_json, 'hwt_primary.' + today_yyyymmdd + '.json')

        corners = json.loads( open( path_json , "rb" ).read() ).get( "corners" )

        print(corners)

        hwtdd_lats = [cp[0] for cp in corners]
        hwtdd_lons = [cp[1] for cp in corners]

        hwtx, hwty = map( hwtdd_lons, hwtdd_lats )
        hwtxy = zip(hwtx, hwty)
        hwtpoly = matplotlib.patches.Polygon( hwtxy , fill=False, edgecolor='red', linewidth=2 )
        P.gca().add_patch(hwtpoly)
    except Exception:
        print('Plotting of HWT daily domain failed; JSON file may not exist in ' + str(dir_json))
